...
func TestAdd_FailNoFolder(t *testing.T) {
	ctx := getTestContext(123)
	...
	err := d.SaveUser(ctx, &pb.StoredUser{
		...
		Folders: []*pbd.Folder{{Name: "other", Id: 456}},
		Auth:    &pb.GramophileAuth{Token: "123"}})
	if err != nil {
		t.Fatalf("Can't init save user: %v", err)
	}
	...
	if err == nil {
		t.Fatalf("Expected error due to missing folder, but got nil")
	}

	if !strings.Contains(err.Error(), "found [other]") {
		t.Errorf("Error does not contain available folders: %v", err)
	}
}
