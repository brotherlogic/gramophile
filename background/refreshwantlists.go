...
func (b *BackgroundRunner) RefreshWantlists(ctx context.Context, d discogs.Discogs, authToken string, enqueue func(context.Context, *pb.EnqueueRequest) (*pb.EnqueueResponse, error)) error {
	...
	user.LastWantlistRefresh = now
	user.LastItemSyncedTime = now
...
func (b *BackgroundRunner) mergeWant(ctx context.Context, userid int32, want *pb.Want, list string) error {
	...
	return b.db.SaveWant(ctx, userid, val, fmt.Sprintf("Updated from refresh wantlist (%v) %v", list, want.GetIntendedState()))
}
