...
func main() {
	...
	case "collection":
		a, b := client.Enqueue(context.Background(), &pb.EnqueueRequest{
			Element: &pb.QueueElement{RunDate: time.Now().UnixNano(), Auth: os.Args[3], Entry: &pb.QueueElement_RefreshCollectionEntry{RefreshCollectionEntry: &pb.RefreshCollectionEntry{Page: 1}}},
		})
		fmt.Printf("%v and %v\n", a, b)
	...
	case "wants":
		a, b := client.Enqueue(context.Background(), &pb.EnqueueRequest{
			Element: &pb.QueueElement{RunDate: 1718597532322472889, Auth: os.Args[3], Entry: &pb.QueueElement_SyncWants{SyncWants: &pb.SyncWants{Page: 1}}},
		})
		fmt.Printf("%v and %v\n", a, b)
	}
}
